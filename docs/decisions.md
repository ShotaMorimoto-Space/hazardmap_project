...
# ADR-009: Hazard / Evacuation Data Strategy
...
本サービスでは、洪水・土砂災害・津波のハザード情報と避難所情報を、インテリア性の高い地図デザイン上に表示する。

一方で、防災情報については以下を満たす必要がある。

- 公的データに準拠する
- 商用利用条件を守る
- ハザード範囲そのものを独自判断で変更しない
- A2 / A1印刷に耐える表現自由度を確保する
- Map Providerに防災ロジックを依存させない
- データの更新性と検証可能性を確保する

また、国のPNG地図タイルを直接利用する方法も検討したが、カテゴリごとの色変更や高品質印刷向けのデザイン自由度が制限される。

## Decision

### Hazard Data

ハザード情報は、国土交通省「国土数値情報」のベクターデータを主データソースとして使用する。
...
国・自治体が定義したハザード区域・属性情報を基礎データとし、本サービス側では区域そのものを変更しない。

### Shelter Data

避難所情報は、国土地理院「指定緊急避難場所・指定避難所データ」を使用する。

国土数値情報の旧「避難施設データ」は、データ年度・利用条件の観点から使用しない。

### Rendering

ハザード情報はPNG地図タイルを主方式とせず、Vector Polygonとして扱う。

背景地図はValidation ReleaseではADR-004に従い、
...
ハザードPolygonは背景地図から独立したレイヤーとして描画する。
...
色・透明度などの視覚表現は変更可能とするが、ハザード区域のgeometryや公的属性の意味は変更しない。

## Validation

国土地理院「重ねるハザードマップ」は主データソースではなく、表示内容の検証・QA用途として使用する。

開発時に、自社表示と公的ハザードマップとの整合性を確認する。

## Data Update

固定周期で全データを再取得するのではなく、最新版の有無を定期チェックする。
...
新版または変更を検知した場合のみデータを更新する。

Validation / MVP段階では自動更新システムを作らず、GitHub Issue等による手動確認でもよい。

## Licensing

公的データの商用利用条件を遵守する。

特に都道府県ごとの追加条件が存在するデータについては、販売対象地域ごとに利用条件を確認する。

津波データについて、現時点で少なくとも以下は事前連絡対象として管理する。

- 京都府
- 長崎県

その他、都道府県固有の利用条件がある場合は個別に確認する。

将来的には、必要に応じて以下のような都道府県別ライセンス管理を検討する。

`prefecture_license_rules.json`

## Attribution

商品本体への出典表記は、デザイン性を損なわない範囲で最小化する。

基本方針：

- ポスター下部に簡易クレジットを配置
- 詳細なデータソース、取得日、利用条件、加工内容はWebサイトの専用ページに掲載
- 正式販売前に「簡易クレジット + Web詳細表記」の形式が各データ利用条件上問題ないか確認する

Webサイトのみへの出典表記で済ませることは、正式な確認が取れるまでは採用しない。

## Principle

本サービスでは独自に災害危険区域を判定しない。

国・自治体が公開しているハザード区域・属性情報を基礎データとし、プロダクト側ではデザイン上の表現のみ変更する。

信頼性と最新性は分けて扱う。

- 信頼性：公的データを基礎にすることで担保する
- 最新性：定期的な更新確認によって担保する

Hazard EngineはMap Providerから独立させ、将来Stadia MapsからMapbox等へ変更しても、防災データ処理を再利用できる構造とする。
